...
2) $\vec{n_1}$ - внешний к $z = \psi(x)$ , но $\vec{n_2}$ - внутренний к $z = \varphi(x)$ 

3) \hyperref[eq10]{$cos\gamma = \frac{1}{\sqrt{1 + \psi_u^2 + \psi_v^2 }}$} для $S_2$ , и $cos\gamma = -\frac{1}{\sqrt{1 + \varphi_u^2 + \varphi_v^2 }} $ для $S_1$

4)$\sqrt{EG - F^2} = \sqrt{(1 + \psi_u^2)(1 + \psi_v^2) - (\psi_u \psi_v)^2} = \sqrt{1 + \psi_u^2 + \psi_v^2 }$

Можно сделать вывод:
$$
\int\limits_{\Omega}\int R(x,y,\psi(x,y))dxdy 
\stackrel{?}{=}
\int\limits_{S_2^+}\int R(x,y,z)  cos\gamma  \sqrt{EG - F^2} dxdy
=
...
\textbf{Осознать, почему там не $\gamma_1 , \gamma_2$!!!}
...
\noindent 4) Путь $\gamma_0$ - граница $\Omega$ , $\gamma_0$ - замкнутая, кусочно-гладкая без самопересечений с \par положительным направление обхода с параматризацией  $\gamma_0 : u(t) , v(t) , t \in [a,b]$

\noindent 5)На $S$ определена номаль $\vec{V}(cos\alpha , cos\beta , cos\gamma)$
...
$$cos\gamma = -\frac{1}{\sqrt{1 + \varphi_u^2 + \varphi_v^2 }} $$
